  ...
  methods: {
    getConnectMessage() {
      ...
      axios({
        ...
        .then((res) => {
          let data = res.data.polymerizationDtos;
          this.allMessage = [];
          for (let i = 0; i < data.length; i++) {
            for (let j = 0; j < data[i].polymerizationServers.length; j++) {
              if (data[i].polymerizationServers[j].round == 0) {
                this.allMessage.push({
                  userIp: "/",
                  userName: "/",
                  department: "/",
                  round: data[i].polymerizationServers[j].round,
                  time: data[i].polymerizationServers[j].startTime,
                });
              }

              if (data[i].polymerizationUsers.length > j) {
                axios({
                  method: "get",
                  url: "/serverRequest/connection/getByUserIdAndTime",
                  headers: { token: this.token },
                  params: {
                    userId: data[i].polymerizationUsers[j].userId,
                    time: data[i].polymerizationUsers[j].startTime,
                  },
                }).then((res) => {
                  this.allMessage.push({
                    userIp: res.data.connection.userIp,
                    userName: res.data.connection.userName,
                    department: res.data.connection.department,
                    round: data[i].polymerizationUsers[j].round,
                    time: data[i].polymerizationUsers[j].endTime,
                  });
                  this.getPageInfo();
                  // this.getUserId();
                });
                // this.allMessage.push({
                //   userIp: data[i].polymerizationUsers[j].,
                // });
              }
            }
          }
