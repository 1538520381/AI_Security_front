...
import DrawLine2 from "@/components/DrawLine2";
...
export default {
  ...
  created() {
    ...
    this.allRound = parseInt(this.$store.state.tableData[0].epochs);
    // console.log(this.allRound);
  ...
  methods: {
      ...
      axios({
        ...
        .then((res) => {
          ...
          for (let i = 0; i < res.data.connections.length; i++) {
            this.dynamicTags.push(res.data.connections[i].userIp);
          }
        })
        .catch((action) => {
          return;
    ...
    dataFormat() {
      ...
      axios({
        method: "get",
        url: "/serverRequest/polymerization/result",
        headers: { token: this.token },
      }).then((res) => {
        console.log(res);
        ...
        if (data != null) {
          ...
          this.nowRound = data[0].mydata.length;
        }
      });
